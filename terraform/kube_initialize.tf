...
#     private_endpoint_id = oci_resourcemanager_private_endpoint.rms_private_endpoint[count.index].id
#     private_ip          = oci_core_instance.bastion_instance[count.index].private_ip
# }

# resource "null_resource" "remote-exec" {
#     count               = var.should_setup_vm ? 1 : 0
#     depends_on = [oci_containerengine_node_pool.pool1]

#     provisioner "remote-exec" {
#         connection {
#             agent = false
#             timeout = "30m"
#             host = data.oci_resourcemanager_private_endpoint_reachable_ip.private_endpoint_reachable_ips[count.index].ip_address
#             user = "opc"
...
#             "oci ce cluster create-kubeconfig --cluster-id ${oci_containerengine_cluster.supremo-oke-cluster[count.index].id} --file $HOME/.kube/config --region ${var.region} --token-version 2.0.0  --kube-endpoint PRIVATE_ENDPOINT --auth instance_principal",
