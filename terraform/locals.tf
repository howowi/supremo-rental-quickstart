locals {
  ...
  opensearch_private_subnet_id = local.should_use_existing_network ? var.existing_private_subnet_id : local.should_config_opensearch_private_subnet ? oci_core_subnet.opensearch_private_subnet[0].id : oci_core_subnet.shared_private_subnet[0].id
  redis_private_subnet_id      = local.should_use_existing_network ? var.existing_private_subnet_id : local.should_config_redis_private_subnet ? oci_core_subnet.redis_private_subnet[0].id : oci_core_subnet.shared_private_subnet[0].id

  should_config_public_subnet = !local.should_use_existing_network && var.should_setup_vm && var.should_config_public_ip_for_vm
  vm_subnet_id                = local.should_use_existing_network ? var.existing_vm_subnet_id : local.should_config_public_subnet ? oci_core_subnet.vm_public_subnet[0].id : local.opensearch_private_subnet_id
